...
# load L2 singlet seurat data 

L2.singlet <- readRDS("./DATA/RDS/L2.singlet.rds")
...
Cell_barcodes_file <- "./DATA/Direct_LARRY/L2/c_post_larry_c2b2l4_N_barcodes.tsv"
LARRY_matrix_file <- "./DATA/Direct_LARRY/L2/c_post_larry_c2b2l4_N.mtx"
LARRY_BC_features_file <- "./DATA/Direct_LARRY/L2/c_post_larry_c2b2l4_N_features.tsv"
Lib_ID_matrix <- "./DATA/Direct_LARRY/L2/c_post_larry_c2b2l4_N_lib.mtx"
Lib_ID_barcodes_file <- "./DATA/Direct_LARRY/L2/c_post_larry_c2b2l4_N_lib_id_barcode.tsv"
...
seurat_obj <- CreateSeuratObject(counts = LARRY_sparse_matrix, assay = "LARRY", project = "LARRY_mtx_L2")
...
L2.LARRY <- subset(seurat_obj, subset = nFeature_LARRY > 0 & nCount_LARRY > 20 & nFeature_Lib_ID > 0 & nCount_Lib_ID > 20) # can tweak the number to change the assignment sensitivity
dim(L2.LARRY)
str(L2.LARRY)

meta.L2.LARRY <- L2.LARRY@meta.data


# Plot scatter plot of LARRY ratio and Lib_ID ratio
pdf(file.path(figures_dir, "L2_LARRY_Lib_ID_Count-Feature_ratio.pdf"),width=8,height=4,paper='special')
max_val <- max(L2.LARRY$nCount_LARRY, L2.LARRY$nFeature_LARRY)
plot1 <- FeatureScatter(L2.LARRY, feature1 = "nCount_LARRY", feature2 = "nFeature_LARRY") +
  coord_cartesian(xlim = c(0, max_val), ylim = c(0, max_val)) + 
  NoLegend() + ggtitle("L2.LARRY") 
max_val_2 <- max(L2.LARRY$nCount_Lib_ID, L2.LARRY$nFeature_Lib_ID)
plot2 <- FeatureScatter(L2.LARRY, feature1 = "nCount_Lib_ID", feature2 = "nFeature_Lib_ID") +
  coord_cartesian(xlim = c(0, max_val_2), ylim = c(0, max_val_2)) + 
  NoLegend() + ggtitle("L2.Lib_ID") 
plot1 + plot2
dev.off()
...
L2.LARRY <- NormalizeData(L2.LARRY, assay = "LARRY", normalization.method = "CLR")
L2.LARRY_counts <- L2.LARRY@assays$LARRY$data
str(L2.LARRY_counts)
dim(L2.LARRY_counts)
L2.LARRY_counts[1:4,1:5]

L2.LARRY <- NormalizeData(L2.LARRY, assay = "Lib_ID", normalization.method = "CLR")
L2.Lib_ID_counts <- L2.LARRY@assays$Lib_ID$data
str(L2.Lib_ID_counts)
dim(L2.Lib_ID_counts)
L2.Lib_ID_counts[1:4,1:5]
...
#'
#' @examples
#' result_Lib_ID <- assign_cell_barcode_classification_adaptive(L2.Lib_ID_counts, base_threshold = 0.2, prefix = "LibID")
#' result_LARRY <- assign_cell_barcode_classification_adaptive(L2.LARRY_counts, base_threshold = 0.2, prefix = "LARRY")
#' head(result_Lib_ID)
#' head(result_LARRY)
#'
...
# usage with two different count matrices. L2.Lib_ID_counts and L2.LARRY_counts are your input matrices for LibID and LARRY barcodes respectively
result_Lib_ID <- assign_cell_barcode_classification_adaptive(L2.Lib_ID_counts, base_threshold = 0.2, prefix = "LibID")
result_LARRY <- assign_cell_barcode_classification_adaptive(L2.LARRY_counts, base_threshold = 0.2, prefix = "LARRY")
...
meta.L2.LARRY$LARRY_first_bc[is.na(meta.L2.LARRY$LARRY_first_bc)] <- "not_detected"
meta.L2.LARRY$LARRY_second_bc[is.na(meta.L2.LARRY$LARRY_second_bc)] <- "not_detected"
meta.L2.LARRY$LARRY_bc_classification[is.na(meta.L2.LARRY$LARRY_first_bc)] <- "not_detected"

meta.L2.LARRY$LARRY_first_bc <- as.factor(meta.L2.LARRY$LARRY_first_bc)
...
meta.L2.LARRY$LibID_first_bc[is.na(meta.L2.LARRY$LibID_first_bc)] <- "not_detected"
meta.L2.LARRY$LibID_second_bc[is.na(meta.L2.LARRY$LibID_second_bc)] <- "not_detected"
meta.L2.LARRY$LibID_bc_classification[is.na(meta.L2.LARRY$LibID_first_bc)] <- "not_detected"

meta.L2.LARRY$LibID_first_bc <- as.factor(meta.L2.LARRY$LibID_first_bc)

# write back to seurat meta
L2.LARRY@meta.data  <- meta.L2.LARRY

# ====== 
# Ensure LibID_first_bc is a character vector
L2.LARRY@meta.data$LibID_first_bc <- as.character(L2.LARRY@meta.data$LibID_first_bc)

# Recode first_bc based on the mapping
L2.LARRY@meta.data$LibID_first_bc <- sapply(L2.LARRY@meta.data$LibID_first_bc, function(bc) {
...
L2.LARRY@meta.data$LibID_first_bc <- factor(L2.LARRY@meta.data$LibID_first_bc)

head(L2.LARRY@meta.data$LibID_first_bc)

# Print the table to verify the recoding
table(L2.LARRY@meta.data$LibID_first_bc)


# === plotting LARRY BC freq ===

# check the LARRY freq table
table(L2.LARRY@meta.data$LARRY_first_bc)

# Calculate the frequency table
LARRY_freq_table <- as.data.frame(table(L2.LARRY@meta.data$LARRY_first_bc))
...
Idents(L2.LARRY) <- "LARRY_bc_classification"
L2.LARRY.singlet <- subset(L2.LARRY, idents = "singlet")
meta.L2.LARRY.singlet <- L2.LARRY.singlet@meta.data

# Calculate the frequency table
LARRY_singlet_freq_table <- as.data.frame(table(L2.LARRY.singlet@meta.data$LARRY_first_bc))
...
# Plot a histogram using ggplot2
pdf(file.path(figures_dir, "L2_filtered_LARRY_singlet_freq_table.pdf"),width=20,height=5,paper='special')
...
pdf(file.path(figures_dir, "L2_filtered_LARRY_singlet_freqabove2_table.pdf"),width=10,height=5,paper='special')
...
Idents(L2.LARRY) <- "LibID_bc_classification"
L2.Lib_ID.singlet <- subset(L2.LARRY, idents = "singlet")
meta.L2.Lib_ID.singlet <- L2.Lib_ID.singlet@meta.data

# Calculate the frequency table
Lib_ID_singlet_freq_table <- as.data.frame(table(L2.Lib_ID.singlet@meta.data$LibID_first_bc))
...
# Plot a histogram using ggplot2
pdf(file.path(figures_dir, "L2_filtered_Lib_ID_singlet_freq_table.pdf"),width=8,height=5,paper='special')
...
pdf(file.path(figures_dir, "L2_filtered_Lib_ID_singlet_freqabove3_table.pdf"),width=6,height=5,paper='special')
...
# ==== Merge LARRY / Lib_ID assignment with L2.singlet seurat obj ======
## merge with L2.singlet with new assay: LARRY and Lib_ID

m <- L2.singlet # m is the obj to hold an obj with merged meta.data 
meta.m <- m@meta.data
...
filtered_LARRY_BC <- rownames(L2.LARRY@assays$LARRY)
...
# pull the selected BCs used for BC assignment
filtered_Lib_ID_BC <- rownames(L2.LARRY@assays$Lib_ID)
...
meta.m <- m@meta.data
colnames(meta.m)
colnames(meta.L2.LARRY)

c <- intersect(row.names(meta.L2.LARRY), row.names(meta.m))

# Merge the LARRY Lib_ID assignment with the L2.singlet data frame using a left join
merged_df <- merge(meta.m, meta.L2.LARRY, by = "row.names", all.x = TRUE)
head(merged_df)
...
table(merged_df$HTO_classification)

LARRY_assignment_result <- merged_df %>%
  filter(HTO_classification %in% c("M-HTO-1", "M-HTO-2", "M-HTO-3", "M-HTO-4", "M-HTO-5", "M-HTO-6", "M-HTO-7", "M-HTO-8")) %>%
...
pdf(file.path(figures_dir, "L2_Assigned_LARRY_BC_freq_c2b2l4_N.pdf"), width=15,height=5,paper='special')
...
pdf(file.path(figures_dir, "L2_Assigned_Lib_ID_BC_freq_c2b2l4_N.pdf"), width=6,height=5,paper='special')
...
write.csv(merged_df, file = "./DATA/RDS/meta_merged_df.Z_L2_c2b2l4_N.csv")

# write back to m
m@meta.data <- merged_df
saveRDS(m, file = "./DATA/RDS/L2.singlet_post_BC_assignement.rds")
...
pdf(file.path(figures_dir, "L2_UMAP_HTO.pdf"),width=15,height=5,paper='special')
...
pdf(file.path(figures_dir, "L2_UMAP_LARRY_status.pdf"),width=10,height=5,paper='special')
DimPlot(df, split.by = "LARRY_status")
dev.off()

pdf(file.path(figures_dir, "L2_UMAP_Lib_ID_status.pdf"),width=10,height=5,paper='special')
DimPlot(df, split.by = "Lib_ID_status")
dev.off()

pdf(file.path(figures_dir, "L2_UMAP_Lib_ID_split.pdf"),width=25,height=5,paper='special')
...
pdf(file.path(figures_dir, "L2_Heatmap.pdf"),width=20,height=20,paper='special')
DoHeatmap(df, features = top10$gene) + scale_fill_gradientn(colors = c("blue", "white", "red")) 
dev.off()

pdf(file.path(figures_dir, "L2_Barplot_LARRY_status.pdf"),width=5,height=5,paper='special')
ggplot(df@meta.data, aes(x=LARRY_status, fill=seurat_clusters)) + geom_bar(position = "fill") + theme_classic()
dev.off()

pdf(file.path(figures_dir, "L2_VlnPlot_Mki67LARRY_status.pdf"),width=5,height=5,paper='special')
VlnPlot(df, features = "Mki67", group.by = "LARRY_status" )
dev.off()

pdf(file.path(figures_dir, "L2_Feature_Krt18.pdf"),width=5,height=5,paper='special')
FeaturePlot(df, features = "Krt18")
dev.off()

pdf(file.path(figures_dir, "L2_Feature_EGFP.pdf"),width=5,height=5,paper='special')
FeaturePlot(df, features = "EGFP")
dev.off()

pdf(file.path(figures_dir, "L2_Feature_C1qa.pdf"),width=5,height=5,paper='special')
FeaturePlot(df, features = "C1qa")
dev.off()
...
FeaturePlot(df, features = "EGFP")
FeaturePlot(df, features = "Krt8")
FeaturePlot(df, features = "C1qa")

pdf(file.path(figures_dir, "Featureplot_BC-265.pdf"),width=6,height=6,paper='special')
FeaturePlot(df, features = "BC-265")
dev.off()

pdf(file.path(figures_dir, "Vlnplot_BC-265.pdf"),width=6,height=6,paper='special')
VlnPlot(df, features = "BC-265", group.by = "LARRY_status" )
dev.off()

pdf(file.path(figures_dir, "Vlnplot_BC-1.pdf"),width=6,height=6,paper='special')
VlnPlot(df, features = "BC-1", group.by = "LARRY_status" )
dev.off()

# save object for next step 
saveRDS(df, file = "./DATA/RDS/L2.singlet_post_BC_assignement_UMAP.rds")
